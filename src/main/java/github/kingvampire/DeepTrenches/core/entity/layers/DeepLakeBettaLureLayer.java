...
@OnlyIn(Dist.CLIENT)
public class DeepLakeBettaLureLayer<T extends LivingEntity> extends LayerRenderer<T, DeepLakeBettaModel<T>> {

    private static final ResourceLocation DEEP_LAKE_BETTA_LURE = new ResourceLocation(MODID,
	    "textures/entity/betta/layers/deep_lake_betta_lure.png");

    public DeepLakeBettaLureLayer(IEntityRenderer<T, DeepLakeBettaModel<T>> entityRendererIn) {
	...
	if (world.getGameTime() % 10 == 0 && entityIn.getAttackingEntity() == null) {
	    this.bindTexture(DEEP_LAKE_BETTA_LURE);
